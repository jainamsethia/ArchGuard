...
# Values for the ``history_status`` field returned by _phase2_commits.
HISTORY_OK = "ok"
...
def _consolidate_communities(communities: dict[str, list[str]]) -> dict[str, list[str]]:
...
def _read_commit_file_sets(repo_root: Path, git: str, window: int | None) -> list[list[str]]:
...
def _phase2_commits(repo_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        _logger.warning(
            "Git history extraction failed for %s: %s", repo_root, history_error,
            exc_info=True,
        )
        graph.clear()
        commits_analyzed = 0
    ...
    if wide_commits_skipped:
        _logger.info(
            ...
            "the co-change graph.", wide_commits_skipped, MAX_FILES_PER_COMMIT,
        )
...
def _phase3_communities(
    ...
    if history_status != HISTORY_OK:
        ...
        _console.print(
            "\n[yellow]! Commit history could not be read "
        ...
        communities = _consolidate_communities(
