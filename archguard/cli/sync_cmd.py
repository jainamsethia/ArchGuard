...
def sync_cache(
    direction: Annotated[SyncDirection, typer.Argument(help="push | pull | both")] = SyncDirection.both,
    bucket: str = typer.Option(..., envvar="ARCHGUARD_S3_BUCKET", help="S3 bucket name"),
    ...
    try:
        ...
        typer.echo("Error: boto3 is required for S3 sync. Install with: pip install boto3", err=True)
        raise typer.Exit(1)
    ...
    if direction in (SyncDirection.pull, SyncDirection.both):
        try:
            ...
            if "404" in str(e):
                console.print("[yellow]⚠ No cache found in S3 (first run). Starting fresh.[/yellow]")
            else:
                raise
