...
def _build_directory_tree(repo_path: Path, max_depth: int = 3) -> str:
    ...
    def walk(directory: Path, depth: int, prefix: str = ""):
        ...
        try:
            paths = sorted([p for p in directory.iterdir() if not p.name.startswith(".") and p.name != "__pycache__"])
        except OSError:
            return
...
async def generate_contract_from_llm(repo_path: Path) -> dict:
    try:
        ...
        raise RuntimeError("ML dependencies are not installed. Run: pip install archguard[ml]")
    ...
    for module in merged.get("modules", []):
        for path in module.get("paths", []):
            if path in louvain_by_path:
                module["coupling_budget"] = louvain_by_path[path].get("coupling_budget", 10)
