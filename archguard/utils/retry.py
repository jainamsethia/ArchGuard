            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if type(e).__name__ == 'RateLimitExceededException':
                        reset_timestamp = getattr(e, 'headers', {}).get('X-RateLimit-Reset', 0)
                        if not reset_timestamp and hasattr(e, 'response'):
                            reset_timestamp = getattr(e.response, 'headers', {}).get('X-RateLimit-Reset', 0)
                        wait = max(0, int(reset_timestamp) - int(time.time())) + 5
                        logger.warning("GitHub rate limit exceeded. Reset in %ds.", wait)
                        if wait < 300:
                            time.sleep(wait)
                            continue
                        else:
                    ...
                    status = getattr(e, 'status', None) or getattr(e, 'response', {}).get('status')
                    if status and status not in retryable_status_codes:
                        raise  # Non-retryable error, re-raise immediately
                    
                    if attempt == max_retries:
                        logger.error("Max retries (%d) exceeded for %s", max_retries, func.__name__)
                        raise
