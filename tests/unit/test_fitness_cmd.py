...
def mock_pipeline(monkeypatch):
    ...
    monkeypatch.setattr("archguard.analysis.layers.AnalysisOrchestrator", MockOrchestrator)
    monkeypatch.setattr("archguard.fitness.evaluator.FitnessFunctionEvaluator", MockEvaluator)
...
def test_fitness_check_critical_fail(mock_pipeline):
    configs = [FitnessFunctionConfig(name="rule_crit", rule="r_crit", severity="critical")]
    results = [FitnessFunctionResult(rule="r_crit", passed=False, details="Crit details")]
    set_mock_data(configs, results)
...
def test_fitness_check_warn_fail_no_flag(mock_pipeline):
    ...
    results = [FitnessFunctionResult(rule="r_warn", passed=False, details="Warn details")]
    set_mock_data(configs, results)
...
def test_fitness_check_json_output(mock_pipeline):
    configs = [
        FitnessFunctionConfig(name="rule_info", rule="r_info", severity="info", rationale="Info reason")
    ]
    results = [
        FitnessFunctionResult(rule="r_info", passed=False, details="Info error")
    ]
    set_mock_data(configs, results)
