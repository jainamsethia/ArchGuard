...
def _mock_analysis_result(violations=None, health_score=100.0, layer_scores=None):
    ...
    return AnalysisResult(
        archdebt=archdebt, violations=violations or [], layer_scores=layer_scores
    )
...
def test_module_import_rule_fails():
    ...
    evaluator._parsed_edges = [
        ImportEdge(
            source_file="src/core/main.py",
            imported_module="src.api.models",
            is_stdlib=False,
...
def test_no_cycles_with_cycle():
    ...
    evaluator._parsed_edges = [
        ImportEdge(
            source_file="src/core/a.py",
            imported_module="src.api.b",
            is_stdlib=False,
            is_third_party=False,
            is_relative=False,
        ),
        ImportEdge(
            source_file="src/api/b.py",
            imported_module="src.core.a",
            is_stdlib=False,
            is_third_party=False,
            is_relative=False,
        ),
...
def test_fan_out_exceeded():
    ...
    evaluator._parsed_edges = [
        ImportEdge(
            source_file="src/api/a.py",
            imported_module="ext1",
            is_stdlib=False,
            is_third_party=True,
            is_relative=False,
        ),
...
def test_evaluate_multiple_rules():
    ...
    rules = ["health_score >= 80", "graph.cycles == 0"]
