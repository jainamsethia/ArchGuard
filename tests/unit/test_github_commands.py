...
def test_execute_re_analyze_direct_call(
    ...
    mock_instance.run.assert_called_once_with(
        changed_files=[], commit_sha="fake_sha", quiet=True
    )
...
def test_execute_suppress_success(mock_post, mock_store_cls, tmp_path: Path) -> None:
    ...
    cmd = SlashCommand(
        command=ArchGuardCommand.SUPPRESS,
        args=["api", "1", "Imports", "from", "db"],
        author="testuser",
        comment_id=1,
    )
    ...
    assert (
        "✅ @testuser Suppressed `api` L1: `Imports from db`"
        in mock_post.call_args[0][1]
    )


@patch("archguard.suppression.store.SuppressionStore")
@patch("archguard.github.client.post_comment")
def test_execute_suppress_usage_error(
    mock_post, mock_store_cls, tmp_path: Path
) -> None:
    from archguard.cli.github_sync_cmd import _execute_suppress
    from archguard.github.commands import SlashCommand
...
@patch("archguard.suppression.store.SuppressionStore")
@patch("archguard.github.client.post_comment")
def test_execute_suppress_invalid_layer(
    mock_post, mock_store_cls, tmp_path: Path
) -> None:
    from archguard.cli.github_sync_cmd import _execute_suppress
    from archguard.github.commands import SlashCommand
