...
def test_run_reinference_drift_ml_exception():
    with (
        patch("archguard.contract.reinference.ReinferenceEngine.check_staleness"),
        patch(
...
def test_run_reinference_propose_exception():
    with (
        patch("archguard.contract.reinference.ReinferenceEngine.check_staleness"),
        patch(
            "archguard.contract.reinference.ReinferenceEngine.should_propose"
        ) as mock_propose,
    ):
        ...
        with pytest.raises(AnalysisError, match="Reinference check failed"):
            _run_reinference(
                ...
                {"m1": [Path("/tmp/a.py")]},
                "sha",
                drift_results={"m1": 0.9},
            )


def test_run_reinference_outer_exception():
    with patch(
        "archguard.contract.reinference.ReinferenceEngine.check_staleness"
    ) as mock_check:
