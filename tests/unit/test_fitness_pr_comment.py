...
def test_build_fitness_section_all_pass_truncation():
    results = [
        {"name": f"Rule {i}", "rule": f"rule{i}", "passed": True}
        for i in range(1, 8)
    ]
    markdown = build_fitness_section(results)
...
def test_build_fitness_section_critical_failure():
    results = [
        {"name": "Crit Rule", "rule": "rule_crit", "passed": False, "severity": "critical", "evidence": "bad"},
...
def test_build_fitness_section_warn_failure():
    results = [
        {"name": "Warn Rule", "rule": "rule_warn", "passed": False, "severity": "warn", "evidence": "watch out"},
    ]
    markdown = build_fitness_section(results)
