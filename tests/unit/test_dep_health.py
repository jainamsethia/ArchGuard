...
from archguard.analysis.deps import analyze_dependencies, DependencyHealthResult, Vulnerability
...
def test_command_generation_requirements(tmp_path: Path):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        assert cmd == ["pip-audit", "--format=json", "-r", str(tmp_path / "requirements.txt")]
