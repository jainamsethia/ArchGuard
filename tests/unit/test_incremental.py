...
def test_get_changed_files_unchanged(tmp_path: Path) -> None:
    ...
    cache_records = {
        "a.py": FileRecord(
            path="a.py", sha256=compute_hash(f1), last_analyzed="2023-01-01T00:00:00Z"
        )
    }
...
def test_get_changed_files_modified(tmp_path: Path) -> None:
    ...
    cache_records = {
        "a.py": FileRecord(
            path="a.py", sha256="fake_hash_123", last_analyzed="2023-01-01T00:00:00Z"
        )
    }
    save_cache(tmp_path, cache_records)
...
def test_save_and_load_cache(tmp_path: Path) -> None:
    ...
    records = {
        "b.py": FileRecord(
            path="b.py", sha256="abcdef", last_analyzed="2023-01-01T00:00:00Z"
        )
    }
...
def test_save_cache_atomic(tmp_path: Path, monkeypatch) -> None:
    ...
    cache_file.write_text(
        '{"old": {"path": "old", "sha256": "123", "last_analyzed": "0"}}'
    )
    write_count = 0
    ...
    with pytest.raises(OSError):
        save_cache(
            tmp_path, {"new": FileRecord(path="new", sha256="456", last_analyzed="1")}
        )
