...
def test_full_analysis_detects_boundary_violation(tmp_path, sample_repo):
    result = runner.invoke(
        app,
        ["analyze", "--repo", str(sample_repo), "--no-llm",
         "--changed-files", "src/core/module_a.py,src/api/module_b.py"],
...
def test_full_analysis_clean_repo_exits_zero(tmp_path, clean_repo):
    result = runner.invoke(
        app,
        ["analyze", "--repo", str(clean_repo), "--no-llm",
         "--changed-files", "src/core/module_a.py"],
        env={"ARCHGUARD_SKIP_ML": "1"},
    )
