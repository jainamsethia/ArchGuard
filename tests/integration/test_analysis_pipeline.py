...
def test_layer1_detects_boundary_violation(temp_project):
    ...
    assert any("Imports" in v.message for v in l1_violations), "Expected violation about api importing db"
...
def test_layer4_violations_can_be_suppressed(tmp_path, temp_project):
    ...
    store.add(module="api", layer=4, message="Duplicate function body", reason="Test suppression for layer 4")
...
def test_suppressed_violation_absent_from_analysis(temp_project):
    ...
    # Add suppression
    store = SuppressionStore(temp_project)
    store.add(module=violation_to_suppress.module, layer=violation_to_suppress.layer, message=violation_to_suppress.message, reason="Suppressing first violation")
    ...
    hashes2 = [make_violation_hash(v.module, v.layer, v.message) for v in result2.violations]
    assert make_violation_hash(violation_to_suppress.module, violation_to_suppress.layer, violation_to_suppress.message) not in hashes2
